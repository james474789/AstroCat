        ...
        try:
            ...
            with open(self.file_path, 'rb') as f:
                ...
                if 'EXIF ISOSpeedRatings' in tags:
                    metadata['gain'] = float(tags['EXIF ISOSpeedRatings'].values[0])
                ...
                if 'EXIF FNumber' in tags:
                    ...
                    try:
                        metadata['aperture'] = float(val.num) / float(val.den) if hasattr(val, 'num') else float(val)
                ...
                if 'EXIF FocalLength' in tags:
                    ...
                    try:
                        metadata['focal_length'] = float(val.num) / float(val.den) if hasattr(val, 'num') else float(val)
                    except (ValueError, ZeroDivisionError, AttributeError):
                        pass
                ...
                if 'EXIF FocalLengthIn35mmFilm' in tags:
                    ...
                    try:
                        metadata['focal_length_35mm'] = float(val) if isinstance(val, (int, float)) else float(str(val).split()[0])
            ...
            with Image.open(self.file_path) as img:
                ...
                if exif:
                    ...
                    if "gain" not in metadata:
                        ...
                        if iso:
                            try:
                                metadata["gain"] = float(iso) if isinstance(iso, (int, float)) else float(str(iso).split()[0])
                    ...
                    if "aperture" not in metadata:
                        ...
                        if fnum:
                            try:
                                metadata["aperture"] = float(fnum[0]) / float(fnum[1]) if isinstance(fnum, tuple) else float(fnum)
                    ...
                    if "focal_length" not in metadata:
                        ...
                        if fl:
                            try:
                                metadata["focal_length"] = float(fl[0]) / float(fl[1]) if isinstance(fl, tuple) else float(fl)
                            except (ValueError, ZeroDivisionError, TypeError):
                                pass
                    ...
                    if "rating" not in metadata:
                        ...
                        if rating:
                            try:
                                rating_val = int(rating) if isinstance(rating, (int, float)) else int(str(rating).split()[0])
                                if 0 <= rating_val <= 5:
                                    metadata["rating"] = rating_val
