...
**Manual Batch Fix**:
- Script: `batch_fix_thumbnails.py`
- Finds all images with null/empty `thumbnail_path`
- Queues regeneration for each using Celery
...
### Recovery
- `batch_fix_thumbnails.py` can re-attempt failed images
...
5. Use `batch_fix_thumbnails.py` for bulk recovery
