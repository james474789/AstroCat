...
from app.schemas.catalog import MessierSchema, NGCSchema, CaldwellSchema, NamedStarSchema, Sh2Schema
...
@router.get("/sh2", response_model=PaginatedResponse[Sh2Schema])
async def list_sh2(
    page: int = Query(1, ge=1),
    page_size: int = Query(50, ge=1),
    constellation: Optional[str] = Query(None),
    ...
    """List Sharpless (Sh2) catalog objects."""
    from app.models.matches import ImageCatalogMatch, CatalogType
    from app.models.image import Image
    from sqlalchemy import select, func, desc
    ...
    stats_subquery = select(
        ...
        ImageCatalogMatch.catalog_type == CatalogType.SH2,
        lights_clause()
    ).group_by(ImageCatalogMatch.catalog_designation).subquery()
    ...
    base_stmt = select(
        ...
        stats_subquery,
        Sh2Catalog.designation == stats_subquery.c.catalog_designation
    )

    if constellation:
        base_stmt = base_stmt.where(Sh2Catalog.constellation == constellation)
    if q:
        normalized_q = q.replace(" ", "")
        base_stmt = base_stmt.where(
            (func.replace(Sh2Catalog.designation, ' ', '').ilike(normalized_q)) |
            (Sh2Catalog.common_name.ilike(f"%{q}%")) |
            (Sh2Catalog.constellation.ilike(f"%{q}%")) |
            (func.replace(Sh2Catalog.source_designation, ' ', '').ilike(normalized_q))
        )
    if has_images:
        base_stmt = base_stmt.where(stats_subquery.c.image_count > 0)
    ...
    if sort_by == "exposure":
        ...
        order_col = Sh2Catalog.ra_degrees
    else:
        order_col = Sh2Catalog.sh2_number
